import { A as require_react, C as resolveQueryBoolean, E as shouldThrowError, M as __toESM, O as timeUntilStale, S as replaceData, T as shallowEqualObjects, _ as isValidTimeout, a as fetchState, b as noop, c as pendingThenable, d as Subscribable, f as notifyManager, h as hashKey, k as timeoutManager, n as useQueryClient, p as environmentManager, r as require_jsx_runtime, s as getDefaultState, u as focusManager, w as resolveStaleTime } from "./QueryClientProvider.js";
...
const getGetApiTodosByIdUrl = (id) => {
	return `/api/todos/${id}`;
};
const getApiTodosById = async (id, options) => {
...
const getGetApiTodosByIdQueryKey = (id) => {
	return [`/api/todos/${id}`];
};
const getGetApiTodosByIdQueryOptions = (id, options) => {
	const { query: queryOptions, fetch: fetchOptions } = options ?? {};
	const queryKey = queryOptions?.queryKey ?? getGetApiTodosByIdQueryKey(id);
	const queryFn = ({ signal }) => getApiTodosById(id, {
		signal,
		...fetchOptions
	});
	return {
...
function useGetApiTodosById(id, options, queryClient) {
...
//#endregion
export { useGetTodos as a, useGetApiTodosById as i, useAddTodo as n, useDeleteTodo as r, getGetTodosQueryKey as t };