...
fn emit_barrel(generated: &Path, include_url_hooks: bool) -> std::io::Result<()> {
    ...
    // Orval repeats its shared helper types (HTTPStatusCodes and friends) verbatim in every tag
    // module that needs them, so two tags returning typed errors both declare the same names. A
    // bare `export *` over both is ambiguous and TypeScript rejects the barrel with TS2308. The
    // first tag to declare a name keeps it; later tags re-export only what they add.
    let mut claimed: HashSet<String> = HashSet::new();
    for tag in &tags {
        let module = generated.join(tag).join(format!("{tag}.ts"));
        let exports = exported_names(&module);
        if exports.iter().all(|export| !claimed.contains(&export.name)) {
            out.push_str(&format!("export * from \"./{tag}/{tag}\";\n"));
        } else {
            // Types and values need separate statements. The bundler erases types before it
            // resolves the barrel, so a type listed in a value `export { .. }` has no runtime
            // binding to find and the build fails with MISSING_EXPORT.
            let fresh = |want_type: bool| -> Vec<&str> {
                exports
                    .iter()
                    .filter(|export| {
                        export.is_type == want_type && !claimed.contains(&export.name)
                    })
                    .map(|export| export.name.as_str())
                    .collect()
            };
            for (names, keyword) in [(fresh(false), "export"), (fresh(true), "export type")] {
                if !names.is_empty() {
                    out.push_str(&format!(
                        "{keyword} {{ {} }} from \"./{tag}/{tag}\";\n",
                        names.join(", ")
                    ));
                }
            }
        }
        claimed.extend(exports.into_iter().map(|export| export.name));
    }
...
/// One name a generated tag module exports.
struct ModuleExport {
    name: String,
    /// True for `type` and `interface` — declarations that vanish at build time and so must be
    /// re-exported with `export type`, not `export`.
    is_type: bool,
}

/// Top-level exported names of a generated tag module, in source order.
///
/// Deliberately a line scanner rather than a parser: orval's output is machine-written in a narrow
/// set of shapes (`export type|const|interface|function|enum|class`, always column zero, one
/// declaration per line), and the only consumer is [`emit_barrel`]'s duplicate check. An
/// unreadable or unrecognized module yields no names, which keeps that caller on a plain
/// `export *` — the behavior from before duplicates had to be handled.
fn exported_names(module: &Path) -> Vec<ModuleExport> {
    ...
    for line in source.lines() {
        ...
        // Modifiers that can sit between `export` and the name.
        while matches!(
            word,
            Some("declare") | Some("abstract") | Some("async") | Some("default")
        ) {
            word = words.next();
        }
        // `enum` is deliberately a value: it survives compilation as a real object.
        let is_type = match word {
            Some("type") | Some("interface") => true,
            Some("const") | Some("let") | Some("var") | Some("function") | Some("enum")
            | Some("class") => false,
            _ => continue,
        };
        let Some(name) = words.next() else { continue };
        // `export const getMe = ...` and `export type foo<T> = ...` both arrive with the
        // declaration's tail still attached when there is no space before the delimiter.
        let name = name
            .split(['=', '<', '(', ':', '{', ';', ','])
            .next()
            .unwrap_or("")
...
"#,
    ...
    #[test]
    fn client_barrel_resolves_names_two_tags_both_declare() {
        // Orval copies its shared HTTP status helpers into every tag module that returns a typed
        // error, so an app with two such tags gets the same names from both. A bare `export *`
        // over both is TS2308; the first tag keeps them and the second re-exports only its own.
        let tmp = tempfile::tempdir().unwrap();
        let generated = tmp.path().join("src/generated");
        for tag in ["me", "orgs"] {
            std::fs::create_dir_all(generated.join(tag)).unwrap();
            std::fs::write(
                generated.join(tag).join(format!("{tag}.ts")),
                format!(
                    "export type HTTPStatusCodes = 200 | 404;\n\
                     export type get{tag}Response = {{ ok: boolean }};\n\
                     export const get{tag} = async () => null;\n"
                ),
            )
            .unwrap();
        }
        ...
        // First tag alphabetically claims the shared name and keeps its star export.
        assert!(barrel.contains(r#"export * from "./me/me";"#));
        // Second names only what it adds, so the duplicate never reaches the barrel twice.
        // Values and types get separate statements — a type in a value re-export has no runtime
        // binding, and the bundler fails on it with MISSING_EXPORT.
        assert!(barrel.contains(r#"export { getorgs } from "./orgs/orgs";"#));
        assert!(barrel.contains(r#"export type { getorgsResponse } from "./orgs/orgs";"#));
        assert!(!barrel.contains(r#"export * from "./orgs/orgs";"#));
    }

    #[test]
    fn exported_names_reads_orval_declaration_shapes() {
        let tmp = tempfile::tempdir().unwrap();
        let module = tmp.path().join("todos.ts");
        std::fs::write(
            &module,
            "import type { Todo } from \"../model\";\n\
             export type HTTPStatusCodes = 200 | 404;\n\
             export type getTodosResponse<T> = T;\n\
        ...
        assert_eq!(
            exports
                .iter()
                .map(|export| (export.name.as_str(), export.is_type))
                .collect::<Vec<_>>(),
            vec![
                ("HTTPStatusCodes", true),
                ("getTodosResponse", true),
                ("TodoParams", true),
                ("getTodosUrl", false),
                ("useTodos", false),
            ]
        ...
        assert!(exported_names(&tmp.path().join("missing.ts")).is_empty());
    }
