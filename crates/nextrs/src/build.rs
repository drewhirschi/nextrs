...
pub fn emit_registry(
    ...
    let routes = deployment_routes(&manifest_dir, &abs_app)?;
    // The CLI checks this receipt against Cargo's build-script artifact. This
    // prevents an older framework or a custom build hook from silently shipping
    // a monolith under several different function names.
    let receipt = serde_json::json!({
        "bundle": std::env::var("NEXTRS_SERVER_BUNDLE").ok(),
        "routes": routes.iter().filter(|r| r.page.exists() || r.route.is_some())
            .map(|r| &r.url_path).collect::<Vec<_>>(),
    });
    let out_dir = std::env::var_os("OUT_DIR").expect("OUT_DIR must be set in build.rs");
    std::fs::write(
        Path::new(&out_dir).join("nextrs-server-bundle.json"),
        receipt.to_string(),
    )?;
...
/// Select server modules using the same manifest as deployment routing.
fn deployment_routes(root: &Path, app: &Path) -> std::io::Result<Vec<DiscoveredRoute>> {
    println!("cargo:rerun-if-env-changed=NEXTRS_SERVER_BUNDLE");
    println!(
        "cargo:rerun-if-changed={}",
        root.join("nextrs.toml").display()
    );
    println!("cargo:rerun-if-changed={}", app.display());
    let routes = discover_routes(app);
    let plan = crate::server_bundles::BundlePlan::discover(root, app, &routes)
        .map_err(std::io::Error::other)?;
    match std::env::var("NEXTRS_SERVER_BUNDLE") {
        Ok(name) => plan.select(&routes, &name).map_err(std::io::Error::other),
        Err(std::env::VarError::NotPresent) => Ok(routes),
...
fn generate_code(routes: &[DiscoveredRoute]) -> String {
    ...
    for route in routes.iter() {
        if route.page.tsx.is_some() {
            let _ = writeln!(out, "    registry.mark_react_page({:?});", route.url_path);
        }
    }
...
"#;
    ...
    fn tsx_shell_error_fallback_catches_resource_load_failures() {
        ...
        assert!(
            shell.contains("}, true);"),
            "error listener must use capture: {shell}"
        );
...
"#;
    ...
    #[test]
    fn selected_codegen_omits_excluded_imports_and_openapi_references() {
        let temp = tempfile::tempdir().unwrap();
        let root = temp.path();
        std::fs::create_dir_all(root.join("app/heavy")).unwrap();
        std::fs::write(
            root.join("app/route.rs"),
            "pub async fn get() -> &'static str { \"light\" }",
        )
        .unwrap();
        std::fs::write(root.join("app/heavy/bundle.toml"), "bundle = 'heavy'").unwrap();
        std::fs::write(
            root.join("app/heavy/route.rs"),
            "use dependency_that_is_not_installed::Engine; #[nextrs::api] pub async fn get() {}",
        )
        .unwrap();
        let routes = discover_routes(&root.join("app"));
        let plan =
            crate::server_bundles::BundlePlan::discover(root, &root.join("app"), &routes).unwrap();
        let code = generate_code(&plan.select(&routes, "default").unwrap());
        assert!(!code.contains("heavy/route.rs"));
        assert!(!code.contains("__nextrs_route_1_route"));
        assert!(code.contains("__nextrs_route_0_route"));
    }
}
