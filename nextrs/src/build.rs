...
pub fn emit_registry(
    ...
    if routes
        .iter()
        .any(|r| r.page.tsx.is_some() || r.not_found.tsx.is_some())
    {
        println!(
...
/// `not-found.{rs,html,tsx}`, if it has one. The render is a [`PageFn`]:
///
/// - `.rs`   — calls the module's `pub async fn render(req) -> String`.
/// - `.html` — baked via `include_str!` + `static_page`.
/// - `.tsx`  — a client-rendered shell (mount div + module script) pointing at
///   the bundle `nextrs::bundle` emits under [`not_found_slug`].
...
"#;
    ...
    fn tsx_page_emits_shell_handler() {
        ...
        assert!(
            code.contains(r#"src=\"/dist/todos.js\""#),
            "expected module script for the slug bundle:\n{}",
            code
        );
