...
- ✅ **VRC-5 (medium)** — resolved by the repo reorg: there is no longer a root `public/` mirror. Vercel's Root Directory is now `site/`, so the CDN serves `site/public/` directly (static assets colocated with the app). Verify `x-vercel-cache: HIT` post-deploy.
...
- ✅ **DEV-2 (high)** — **superseded by the repo reorg.** Originally the two runners were kept separate because the old repo-root layout wasn't a standard single-app layout, so `cargo-nextrs-dev` couldn't watch it and `xtask` was retained as the workspace watcher. The reorg made `site/` a self-contained standard-layout app (`app/`, `src/`, `client/`, `build.rs`), so `cargo-nextrs-dev` now drives it directly: `cd site && cargo dev`. **`xtask` was removed** — the site dogfoods the shipped runner. (Tradeoff accepted: editing `crates/nextrs/src` no longer auto-triggers a site rebuild; restart or touch a site file.)
